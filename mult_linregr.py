...
def std_deviation(X,ind):
	...
	tempsum=float(0)
	for columnIndex in range(0,ind-1):
...
#	print sum(samples[row][i]*theta[i] for i in range(len(theta))) , "   " ,row, ". data icin hesap"
'''
def compute_costMSE():
	sumofErrors=float(0)
...
def compute_costMSE():
	...
	for row in range(0, NUMBER_OF_TEST+1):
		...
		sumofErrors = sumofErrors + ((sum(normalizedTestData[row][i]*theta[i] for i in range(len(theta)))- test[row][36]) ** 2)
	sumofErrors = sumofErrors / NUMBER_OF_TEST
...
def compute_costMAE():
	sumofErrors=float(0)
	for row in range(0, NUMBER_OF_TEST+1):
		sumofErrors = sumofErrors + abs((sum(normalizedTestData[row][i]*theta[i] for i in range(len(theta))) - test[row][36]))
	sumofErrors = sumofErrors / NUMBER_OF_TEST
